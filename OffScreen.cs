...
public class OffScreen : MonoBehaviour
{
    ...
    void Update()
    {
        //CalculateFrustumPlanes() returns 6 planes that form the camera view
        Plane[] planes = GeometryUtility.CalculateFrustumPlanes(Camera.main);

        //test if the sprite is rendered in these planes
    ...
    void CheckTile()
    {
        if (this.tag == MyTags.ROAD)
        {
            Change(ref MapGenerator.instance.last_Pos_Of_Road_Tile,
                ...
                ref MapGenerator.instance.last_Order_Of_Road_Tile);
        }
